...
export const SubSectionModal = ({modalData, setModalData, add=false, view=false,edit=false}) => {
  if(view){
  console.log("videourl", modalData.videoUrl);
}
  ...
  const onSubmit = async (data)=> {
    ...
    if(result){
      ...
      const updatedCourse = {...course, courseContent: updatedCourseContent};
      dispatch(setCourse(updatedCourse));
      console.log("printing course after creating subsection", course);
    }
    setModalData(null);
    setLoading(false);
  }
          ...
          </div>
          {
            !view && (
              <div className="flex justify-end">
                <IconBtn disabled={loading}  text={ loading ? "Loading..." : edit ? "Save Changes": "Save"}></IconBtn>
              </div>
            )
          }
        </form>
